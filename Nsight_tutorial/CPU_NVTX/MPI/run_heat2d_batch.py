...
heat_template = load_template("heat2d_template.c")
...
with open(CONFIG_FILE) as f:
    for line in f:
        ...
        nx, ny, nt, nomp = map(int, line.split())

        tag = f"{nx}_{ny}_{nt}_{nomp}"
        exe = f"heat2d_{tag}"
        jobname = f"heat-{tag}"
