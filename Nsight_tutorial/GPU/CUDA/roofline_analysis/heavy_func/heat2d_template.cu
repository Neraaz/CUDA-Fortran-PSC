...
// ================= CUDA KERNEL =================
__global__
void heat_step_heavy(double *u, double *u_new,
                   int nx, int ny,
                   double dx, double dy,
                   double alpha, double dt)
{
    ...
    if (i > 0 && i < nx-1 && j > 0 && j < ny-1) {
        ...
        // We take the current temperature and "grind" it through
        // 500 iterations of trigonometry. This stays in the registers
        // and doesn't touch the RAM.
        double temp = u[idx];
        for (int k = 0; k < 500; k++) {
            // Using sin, cos, and exp forces the GPU to use its
            // "Special Function Units" (SFU), which are very heavy.
            temp = sin(temp) * cos(temp) + exp(-0.01 * temp);
        }

        // We ensure the result 'temp' is used so the compiler
        // doesn't optimize the loop away.
        double dummy_factor = (temp > 1000.0) ? 1.0 : 1.0;
        // -------------------------------

        u_new[idx] = (u[idx] * dummy_factor) +
            alpha * dt * (
...
int main() {
    ...
    for (int n = 0; n < nt; n++) {
        ...
        heat_step_heavy<<<grid, block>>>(d_u, d_u_new,
                                   nx, ny,
                                   dx, dy,
                                   alpha, dt);
